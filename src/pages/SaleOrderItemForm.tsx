...
const SaleOrderItemForm = () => {
    ...
    return (
        <div>
            ...
            <Header />
            {
                id ? (<h2 className='text-center font-bold text-2xl mt-10'>Update order</h2>) : (<h2 className='text-center font-bold text-2xl mt-10'>Create new Oder</h2>)
            }

            {id ? (<table className='mt-10 w-[80%] mx-auto border-collapse'>
                <thead>
                    <tr>
            ...
            {id ? (<h2 className="font-semibold text-lg w-[80%] mx-auto mt-20">New values</h2>) : (<></>)}
            <form onSubmit={formik.handleSubmit} className=' flex flex-col w-full mx-auto items-center'>
                <div className='flex flex-col w-[80%]'>
                    <label className='font-bold mt-1' htmlFor="productName">Product Name</label>
                    <input
                        id="productName"
                        name="productName"
                        type="text"
                    ...
                    {formik.touched.productName && formik.errors.productName ? (
                        <div className='mt-1 text-red-500 text-sm uppercase'>{formik.errors.productName}</div>
                    ) : null}
                </div>
                <div className='flex flex-col w-[80%]'>
                    <label className='font-bold mt-1' htmlFor="description">Description</label>
                    <input
                        id="description"
                        name="description"
                        type="text"
                    ...
                    {formik.touched.description && formik.errors.description ? (
                        <div className='mt-1 text-red-500 text-sm uppercase'>{formik.errors.description}</div>
                    ) : null}
                </div>
                <div className='flex flex-col w-[80%]'>
                    <label className='font-bold mt-1' htmlFor="quantity">Quantity</label>
                    <input
                        id="quantity"
                        name="quantity"
                        type="number"
                    ...
                    {formik.touched.quantity && formik.errors.quantity ? (
                        <div className='mt-1 text-red-500 text-sm uppercase'>{formik.errors.quantity}</div>
                    ) : null}
                </div>
                <div className='flex flex-col w-[80%]'>
                    <label className='font-bold mt-1' htmlFor="price">Price</label>
                    <input
                        id="price"
                        name="price"
                        type="number"
                    ...
                    {formik.touched.price && formik.errors.price ? (
                        <div className='mt-1 text-red-500 text-sm uppercase'>{formik.errors.price}</div>
                    ) : null}
                </div>
                <div className='my-5'>
                    <label className='font-bold mt-1 mr-5' htmlFor="sale">Sale</label>
                    <input
                        id="sale"
                        name="sale"
                        type="checkbox"
                ...
                <button className=' bg-blue-400 py-2 px-4 rounded-md  text-white hover:bg-blue-600 duration-300 transition-all' type="submit">{id ? 'Update' : 'Create'}</button>
            </form>
        </div>
    );
};
